...
评分要点：
- 新增带分数的 inbox 事件必须在 `paper` 内完整填写 `score_scheme: "daily-v1"`、
  `score_relevance`（0–4）、`score_generality`（0–3）、`score_innovation`（0–3）和
  `score_total`（三项之和）；同时保存分项理由。不能只填总分或省略 scheme，缺乏依据时不估填。
- 统一论文表在 `assessments` 中按来源和评分方案保留评分，不设跨方案的顶层 `score_total`。
  既有日报和历史方案保持原始分数，迁移不重评；比较时必须先限定同一方案。
...
核心入选阈值仍为 7 分。另加 `research_utility`，避免把“值得阅读”和“已证明基座能力”混为一谈：

| 标签 | 用途 |
|---|---|
| `core_model` | 架构、预训练与适配的直接对照 |
| `data_evaluation` | 数据、基准、协议及评测可信度 |
| `diagnostic` | 失败模式、负结果、泛化边界、观测或表示能力检查 |
| `transferable_method` | 可迁移到项目的技术方法 |
| `extended_reading` | 未达核心阈值但有明确参考理由 |

一篇可含多个用途；它们不是额外加分项。B/C 方向的诊断与评测研究不要求自己训练多物理基座模型，
应按其揭示的普适问题、实验设计与证据强度评估通用性，不能因为缺少多物理预训练就自动扣至低分。
附录论文不计核心新增，是否已在日报交付与当前分数分别保存。

`reading_status` 使用 `abstract_checked`、`fulltext_checked`、`code_checked`，未知为 `null`。
代码链接存在不代表已运行代码；运行验证需另附命令、版本和结果证据。
每个重要判断链接具体证据；`author_statement` 保留作者表述，不能当成执行者已复验结论。
历史未核验记录保持原始状态，不批量填“正文已核验”。

## 五、重点论文比较字段

以下字段用于带证据的 discovery/assessment 事件，未知填 `null`。
先覆盖最相关论文，再扩充目录；不从标题、模型名称或摘要中推断未披露的训练与评测设置。

| 字段 | 应记录内容 |
|---|---|
| `representation` | 网格/点云/图、token 化、物理场表示、坐标处理 |
| `conditioning` | 方程、参数、初值、边界、几何如何输入 |
| `pretraining` | 目标函数、数据规模、模型规模、预算和训练任务 |
| `training_pdes` / `evaluation_pdes` | 训练与评测 PDE 家族，分别列出 |
| `generalization_axes` | 未见 PDE、几何、分辨率、参数或边界；区分训练内变化 |
| `evaluation` | 划分、指标、基线、预算、零样本/少样本/完整微调协议 |
| `limitations` | 适用范围、失败模式、尚未验证的结论 |
| `code_url` / `data_url` | 实际核验的链接，权重及许可证情况附证据说明 |
| `research_utility` / `reading_status` | 研究用途与已经完成的核验层次 |

比较时维持原评测口径：不能把分辨率变化当成未见 PDE，把单任务重训当成通用预训练，
把同网格重构当成求解能力。不同指标、预算和数据划分不直接排名。
生成的 RESEARCH_COMPARISON.md 是当前已知字段的视图，不代表所有重点论文已完成上述核验。

每周综合包含：新增证据、已有判断如何变化、值得检验的假设及所需对照。
若只是新摘要而无可比较实验证据，应明确保留为待核验线索。
