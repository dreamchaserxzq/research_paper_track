...
| 维度 | 分值 | 说明 |
|------|------|------|
| **主题相关度** | 0–4 | 与"PDE 基座大模型"这一核心主题的契合程度。单一 PDE、单一物理的一次性模型给低分；面向通用/多物理/可迁移的工作给高分。 |
| **基座特性强度** | 0–3 | 是否体现基座模型的关键属性：多物理/多任务预训练、跨方程/几何/分辨率可迁移、规模化（大数据/大参数/缩放规律）证据。 |
| **创新性** | 0–3 | 是否提出新架构、新预训练范式、新基准/数据集，或新理论（泛化界、缩放律等）。 |

评分要点：
- 只保留 **总分 ≥ 7** 的论文；数量不设下限，0 篇也正常，**绝不用已推送过的论文凑数**。
- 未达阈值但有价值的工作，可放入"附：本期未入选但值得关注的论文"。
- 纯粹的单方程 PINN/神经算子求解器（无基座/迁移属性）通常 ≤ 6 分，除非其方法对基座模型有明确启发。

---

## 三、方向标签写法

日报中每篇论文的"分类标签"字段写为：`[A/B/C 方向] | [arXiv 分类]`，例如 `A | cs.LG`。
一篇论文可跨方向，用 `A/B` 等表示，主方向写在前。
